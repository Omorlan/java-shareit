...
public class OwnerWaitingBookingsHandlerBooking extends AbstractOwnerBookingStateHandler {
    private final BookingRepository bookingRepository;

    @Override
    public List<Booking> handle(BookingRequest request) {
        if (request.getState() == BookingState.WAITING) {
            return bookingRepository.findAllByItemOwnerIdAndStatusOrderByStartDesc(request.getUserId(), BookingStatus.WAITING);
        }
