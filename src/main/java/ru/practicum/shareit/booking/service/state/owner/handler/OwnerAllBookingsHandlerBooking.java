...
public class OwnerAllBookingsHandlerBooking extends AbstractOwnerBookingStateHandler {
    private final BookingRepository bookingRepository;

    @Override
    public List<Booking> handle(BookingRequest request) {
        if (request.getState() == BookingState.ALL) {
            return bookingRepository.findAllByItemOwnerIdOrderByStartDesc(request.getUserId());
        }
